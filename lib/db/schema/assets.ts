...
export const assets = pgTable(
  "assets",
  {
    ...
    batchNumber: varchar("batch_number", { length: 128 }).notNull(),
    originOrganisationId: varchar("origin_organisation_id", { length: 64 })
      .notNull()
      .references(() => organisations.id, { onDelete: "restrict", onUpdate: "cascade" }),
    currentStage: varchar("current_stage", { length: 100 }).notNull(),
    currentCustodianOrganisationId: varchar("current_custodian_organisation_id", { length: 64 })
      .notNull()
      .references(() => organisations.id, { onDelete: "restrict", onUpdate: "cascade" }),
